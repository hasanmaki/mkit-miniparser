...
from src.exception import HTTPConnectionError, HttpResponseError
from src.log_utils import logger


def build_http_client(settings: AppSettings) -> httpx.AsyncClient:
    """Factory untuk httpx.AsyncClient."""
    return httpx.AsyncClient(
        base_url=settings.http.base_url,
        timeout=httpx.Timeout(settings.http.timeout, read=settings.http.timeout * 2),
        headers={
...
async def close_http_client(client: httpx.AsyncClient) -> None:
    """Tutup koneksi httpx.AsyncClient."""
    await client.aclose()
